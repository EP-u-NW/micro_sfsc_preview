/**
 * @brief Makes sure that the platform dependencies are impelemented properly.
 *
 * This example should be run after you impelmented the functions from
 * sfsc_platform.h and sfsc_sockets.h on a new platform, to see if they seem
 * good.
 */
 ...
 *
 * The configured host is a delayed echo server, it will return whaterver data
 * you send it with a delay of 10 seconds. This delay is necessary to test if
 * calls to socket_read behave nonblocking: They should return if there are no
 * bytes readable instead of waiting for readable bytes.
 *
 * If the delayed echo server is down, this test will fail. You should then
 * impelement your own delayed echo server.
 *
...
int embedding_test() {
    ...
    sfsc_int16 handle =
        socket_connect(delayed_echo_server_host, DELAYED_ECHO_SERVER_PORT);
    if (handle >= SOCKET_OK) {
        ...
        *time = time_ms();
        ...
        console_print_uint32(
            (sfsc_uint32)(time_ms() - *((sfsc_uint64 *)in_buff)));
        ...
        console_println_char(
            "Calls without data should be > 100 to prove that nonblocking I/O "
            "is working");
        if (calls_without_data > 100) {
            ...
            console_println_char(
                "Note: Try running this example again, the first time it is "
...
int example_main() { return embedding_test(); }
