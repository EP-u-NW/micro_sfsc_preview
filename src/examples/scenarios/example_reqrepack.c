 ...
 * defined)
 * 7. Receive the automatically sended ack message on the server and clean up to
...
static sfsc_buffer answer_payload_buffer = {
    (const sfsc_uint8 *)&long_running_task_result, 8};

static void on_ack(sfsc_adapter *a, sfsc_server *s, sfsc_bool timeout,
                   void *mapping_arg) {
#ifdef ENABLE_PRINTS
    if (timeout) {
 ...
 *
 * @returns 1 if the task could be started, 0 if there is already another task
 * running
 */
static bool long_running_task_start_nonblocking(sfsc_uint64 parameter) {
    if (global_reply_topic_actual_length ==
        0)  // if there is already a task running, this check will be false
    {
...
static void long_running_task_step_nonblocking() {
    if (long_running_task_steps_needed != 0) {
        ...
        if (long_running_task_steps_needed == 0) {
            ...
            // we need to make sure that the payload we are going to answer is
            // valid until we receive the on_ack call. Luckly, this is quite easy
            // in this example since the long_running_task_result is already a
            // global variable, we just need to interprete it as a sfsc_uint8* to
            // use it as a payload. The second thing we need to make sure is,
            // that since we are going to pass a pointer to a sfsc_buffer (and
            // not a sfsc_buffer itselfe, keyword: mutability), the sfsc_buffer
            // we point to is also valid till to on_ack call. So we can not
            // declare it here, but use a staticly declared buffer. Now, go
            // ahead and take a look at the global variable
            // answer_payload_buffer, and hopefully, it will make sense now. What
            // we can do here, is to create a sfsc_buffer that holds the reply
            // topic (since the reply_topic parameter is not a pointer). We also
            // have to make sure, that the content of the reply_topic is valid
            // until on_ack is called, which it will be, since again, we are
            // using refering to a global variable.
            sfsc_buffer reply_topic_buffer = {
                (const sfsc_uint8 *)global_reply_topic,
                global_reply_topic_actual_length};
            sfsc_int8 op_result = answer_request(
                &adapter, &server, global_expected_reply_id, reply_topic_buffer,
                &answer_payload_buffer, NULL, on_ack);
...
static void long_running_task_cancel() { long_running_task_steps_needed = 0; }
...
static void on_request_for_server(sfsc_adapter *a, sfsc_server *s,
    ...
    // Since we only got one service active we know to whom this request was
    // made. We are now going to start a long running, non blocking task which
    // will requrie multiple steps to solve.
    sfsc_uint64 task_parameter = *(const sfsc_uint64 *)payload.content;
    // Note that this example is only capable of one simultanious task, since
    // the function we are invoking will use global memory to store state
    // varaibles. If you need to serve multiple requests at the same time, you
    // need to come up with a solution for your use-case, e.g. using an struct
    // type for state memory and declarding a global array of that struct type.
    if (long_running_task_start_nonblocking(task_parameter)) {
        // Once the task is compelted, it will answer the request.
        // To answer the request we need to know the reply_topic and the
        // expected_reply_id but these two things are only valid in this callback
        // (see documentation). We can solve this problem 2 ways: 1. either copy
        // both things somewhere global OR
        // 2. pause the user task. Option 2 is discussed in the 'The Execution
        // model' section of the readme, so this example will demonstrate
        // option 1. Since we are not pausing the user task to this:
        *b_auto_advance = 1;
        ...
        if (reply_topic.length <= MAX_GLOBAL_REPLY_TOPIC_LENGTH) {
            ...
            memcpy(global_reply_topic, reply_topic.content,
                   global_reply_topic_actual_length);
        } else {
#ifdef ENABLE_PRINTS
            console_println_char(
                "Reply topic to long to topic, increase "
                "MAX_GLOBAL_REPLY_TOPIC_LENGTH");
#endif
...
#ifdef ENABLE_PRINTS
        console_println_char(
            "There is already a request running, can not serve a second one!");
#endif
    }
}

static void on_answer(sfsc_adapter *a, sfsc_buffer payload, sfsc_bool timeout,
                      void *mapping_arg, sfsc_bool *b_auto_advance) {
    if (!timeout) {
...
static sfsc_int8 send_a_request() {
    ...
    // sfsc_adapter.h).
    sfsc_buffer request_topic = {(const sfsc_uint8 *)server.service_id.id,
                                 UUID_LEN};
    ...
    return request(&adapter, request_topic, payload_buffer, 30000, on_answer,
                   NULL);
}
