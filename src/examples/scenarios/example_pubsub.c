/**
 * @brief Demonstrates the usage of a publisher and a subscriber.
 * 
 ...
 * 4. While the publisher has at least one subscriber, publish messages every PUBLISHING_INTERVAL_MS
 * 5. Print the messages the subscriber receives (only if ENABLE_PRINTS is defined)
 */
...
static void on_subscription_change(sfsc_adapter *a,
                                   sfsc_publisher *p,
                                   ...
                                   sfsc_uint8 current_subscribers)
{
    ...
    {
        next_publish_time = time_ms(); //Indicates that we want to publish messages
    }
    else
    {
        next_publish_time = 0; //Indicates that we don't want to publish messages
    }
}
...
{
    //We only subscribed one publisher, so we know that the payload is a sfsc_uint32
...
#endif
    *b_auto_advance = 1; //We don't need the payload outside of this callback, so there is no need in entering the user task pause state
}
...
//We got here from the register_publisher call
...
#endif
        //Since the publisher is now part of the service registry, we can create a subscriber
        //and listen to it. Usually, we would query the service registry, but we are using
        //a shortcut here: We already know the topic of our publisher, since we used
        //SERVICE_TOPIC_AUTOGEN, the publishers topic is equal to it's service id
        //(as docummented in sfsc_adapter.h).
        ...
        //After this call, the publisher should detect that it has a subscriber, so
        //on_subscription_change is invoked.
...
static sfsc_int8 register_publisher_service()
{
    ...
    //We want the topic to be automatically generated, so normally we would use the SERVICE_TOPIC_AUTOGEN macro.
    //But the macro can only be used during struct declaration, so we need to set the already existing
    //topic struct of our publisher manually to the values from the macro.
    ...
    //We use the normal (not the unregistered) version of the register_publisher function,
    //since we want the publisher to show up in the service registry.
    //We also give the publisher a name, but ignore the other fields for now.
    //Lastly, we use a callback to get notified once our publisher was written into the service registry.
    const char publisher_name[] = "Hello_World_Publisher";
    sfsc_buffer name = {(const sfsc_uint8 *)publisher_name, strlen(publisher_name)};
    sfsc_buffer empty = sfsc_buffer_DEFAULT_INIT;
    return register_publisher(&adapter, &publisher, name, empty, empty, on_publisher_created);
}
...
static sfsc_int8 publish_message_if_necessary()
{
    ...
    {
        sfsc_buffer message = {(const sfsc_uint8 *)&published_message_number, 4}; //We use the message number as payload
...
static sfsc_bool example_step()
{
    ...
    if (op_result == SFSC_OK || op_result == W_MESSAGE_DROP) //Message drop is just a warning, not an error
    {
        ...
        if (stats->state >= SFSC_STATE_OPERATIONAL)
        {
            if (first_time)
            {
                first_time = 0;
#ifdef ENABLE_PRINTS
                console_println_char("SFSC Handshake done, adapter operational");
                ...
                console_println();
#endif
                //Since the adapter is now operational, lets register a publisher.
...
{
    //Start a session
    sfsc_int8 op_result = start_session_bootstraped(&adapter, host, CONTROL_PUB_PORT);
    if (op_result == SFSC_OK)
    {
